...
def monitor_callback(
    ...
    if ctx.invoked_subcommand is None:
        run_monitor(refresh_interval=refresh_interval, iterations=iterations)
...
def build_job_stats_panel(stats: dict) -> Panel:
    ...
    for state_name, count in counts.items():
        ...
        bar_str = f"[{color}]" + "█" * bar_len + "[/]" + "[bold black]" + "█" * (30 - bar_len) + "[/]"
...
def build_jobs_table_panel(jobs: list) -> Panel:
    table = Table(
        show_header=True,
    ...
    sorted_jobs = sorted(jobs, key=job_sort_key)[:12]
    ...
    return Panel(
        table,
        title="[bold green]Recent & Active Jobs Summary[/bold green]",
        border_style="green",
        padding=(0, 1),
    )


def build_dashboard_layout(repository: JobRepository, queue_service: QueueService) -> Group:
    ...
    table_panel = build_jobs_table_panel(jobs)
    footer = Align.center(
        Text.from_markup("[dim white]Press [bold red]Ctrl+C[/bold red] to exit Live Monitor | Auto-refreshing in real-time[/dim white]")
    )
...
def run_monitor(refresh_interval: float = 1.0, iterations: Optional[int] = None):
    db = SessionLocal()
    ...
    try:
        with Live(
            build_dashboard_layout(repository, queue_service),
            console=console,
            ...
            while True:
                ...
                db.expire_all()
                live.update(build_dashboard_layout(repository, queue_service))
                count += 1
